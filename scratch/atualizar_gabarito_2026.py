...
atualizados = 0
for num_str, gab in user_gabarito.items():
    k = f"REVALIDA-2026_1_caderno_1_{num_str}"
    if k in cache:
        gab_antigo = cache[k].get("gabarito")
        if gab_antigo != gab:
            print(f"Atualizando {k}: {gab_antigo} -> {gab}")
            cache[k]["gabarito"] = gab
            atualizados += 1
    else:
        cache[k] = {"gabarito": gab, "explicacao": ""}
        atualizados += 1

print(f"Total de questões atualizadas no cache: {atualizados}")
